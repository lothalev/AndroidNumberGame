...
/**
 * An activity representing a single High score detail screen. This activity is
 * only used on handset devices. On tablet-size devices, item details are
 * presented side-by-side with a list of items in a
 * {@link HighscoreListActivity}.
 * <p>
 * This activity is mostly just a 'shell' activity containing nothing more than
 * a {@link HighscoreDetailFragment}.
 */
public class HighscoreDetailActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			arguments.putString(
					HighscoreDetailFragment.ARG_ITEM_ID,
					getIntent().getStringExtra(
							HighscoreDetailFragment.ARG_ITEM_ID));
			HighscoreDetailFragment fragment = new HighscoreDetailFragment();
			fragment.setArguments(arguments);
			getSupportFragmentManager().beginTransaction()
					.add(R.id.highscore_detail_container, fragment).commit();
		}
	}
